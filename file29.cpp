...
bool AppInit(int argc, char* argv[])
{
    ...
    {
        fprintf(stderr,"\nError: Data directory %s needs to exist before calling kaicoind. Exiting...\n\n",mapArgs["-datadir"].c_str());
        return false;        
    }
        ...
        (mc_gState->m_Params->NetworkName() == NULL))
    {
        ...
        {
            strUsage += "\n" + _("Usage:") + "\n" +
                  "  kaicoind <blockchain-name> [options]                     " + _("Start KaiCoin Core Daemon") + "\n";
...
//                delete mc_gState;
    ...
    if(err)
    {
        fprintf(stderr,"ERROR: Couldn't read configuration file for blockchain %s. Please try upgrading KaiCoin. Exiting...\n",mc_gState->m_Params->NetworkName());
        delete mc_gState;                
        return false;
    }
    ...
    if(err)
    {
        ...
        {
            fprintf(stderr,"\nERROR: Couldn't initialize permission database for blockchain %s. Please restart kaicoind with reindex=1.\n",mc_gState->m_Params->NetworkName());                        
        }
        else
        {
            fprintf(stderr,"\nERROR: Couldn't initialize permission database for blockchain %s. Probably kaicoind for this blockchain is already running. Exiting...\n",mc_gState->m_Params->NetworkName());
        }
        delete mc_gState;                
        return false;
    }
    ...
    if(mc_gState->m_NetworkParams->GetParam("protocolversion",&size) != NULL)
    {
        ...
        if(mc_gState->IsSupported(protocol_version) == 0) 
        {
            ...
            {
                fprintf(stderr,"ERROR: The protocol version (%d) for blockchain %s has been deprecated and was last supported in KaiCoin %s\n\n",
            ...
            {
                fprintf(stderr,"ERROR: Parameter set for blockchain %s was generated by KaiCoin running newer protocol version (%d)\n\n",
                        mc_gState->m_Params->NetworkName(),protocol_version);                        
                fprintf(stderr,"Please upgrade KaiCoin\n\n");
                delete mc_gState;                
                return false;
            }
        }
    ...
    switch(mc_gState->m_NetworkParams->m_Status)
    {
        ...
        case MC_PRM_STATUS_MINIMAL:
            if(mc_gState->GetSeedNode() == NULL)
            {
                ...
                fprintf(stderr,"  kaicoin-util create %s\n",mc_gState->m_Params->NetworkName());
                fprintf(stderr,"  kaicoin-util clone <old-blockchain-name> %s\n",mc_gState->m_Params->NetworkName());
                fprintf(stderr,"\nAnd rerun kaicoind %s\n\n\n",mc_gState->m_Params->NetworkName());                        
                fprintf(stderr,"If you want to connect to existing blockchain please specify seed node:\n\n");
                fprintf(stderr,"  kaicoind %s@<seed-node-ip>\n",mc_gState->m_Params->NetworkName());
                fprintf(stderr,"  kaicoind %s@<seed-node-ip>:<seed-node-port>\n\n\n",mc_gState->m_Params->NetworkName());
                delete mc_gState;                
                return false;
            }
            break;
        ...
        case MC_PRM_STATUS_INVALID:
            ...
            fprintf(stderr,"  kaicoin-util clone %s <new-blockchain-name>\n",mc_gState->m_Params->NetworkName());
            delete mc_gState;                
            return false;
    ...
    SelectKaiCoinParams(mc_gState->m_Params->NetworkName());
...
/*        
        ...
        if (fDaemon)
        {
            fprintf(stdout, "KaiCoin server starting\n");
...
// Setting Argv for Kaicoin
    char *temp = (char*)malloc(sizeof(char)*255);
    sprintf(temp, "kaicoin@%s", argv[1]);
    argv[1] = temp;
//
