...
CzCCIPWallet::CzCCIPWallet(std::string strWalletFile)
{
    ...
    if (fFirstRun) {
        uint256 seed;
        if (walletdb.ReadZCCIPSeed_deprecated(seed)) {
            //Update to new format, erase old
            seedMaster = seed;
            hashSeed = Hash(seed.begin(), seed.end());
            if (pwalletMain->AddDeterministicSeed(seed)) {
                if (walletdb.EraseZCCIPSeed_deprecated()) {
                    LogPrintf("%s: Updated zCCIP seed databasing\n", __func__);
                    fFirstRun = false;
                } else {
...
bool CzCCIPWallet::SetMasterSeed(const uint256& seedMaster, bool fResetCount)
{
    ...
    if (fResetCount)
        walletdb.WriteZCCIPCount(nCountLastUsed);
    else if (!walletdb.ReadZCCIPCount(nCountLastUsed))
        nCountLastUsed = 0;
...
void CzCCIPWallet::Lock()
{
    seedMaster = 0;
}

void CzCCIPWallet::AddToMintPool(const std::pair<uint256, uint32_t>& pMint, bool fVerbose)
{
    mintPool.Add(pMint, fVerbose);
}

//Add the next 20 mints to the mint pool
void CzCCIPWallet::GenerateMintPool(uint32_t nCountStart, uint32_t nCountEnd)
{
    ...
    for (uint32_t i = n; i < nStop; ++i) {
        ...
        CKey key;
        SeedToZCCIP(seedZerocoin, bnValue, bnSerial, bnRandomness, key);
...
bool CzCCIPWallet::LoadMintPoolFromDB()
{
...
void CzCCIPWallet::RemoveMintsFromPool(const std::vector<uint256>& vPubcoinHashes)
{
    for (const uint256& hash : vPubcoinHashes)
        mintPool.Remove(hash);
}

void CzCCIPWallet::GetState(int& nCount, int& nLastGenerated)
{
...
void CzCCIPWallet::SyncWithChain(bool fGenerateMintPool)
{
...
bool CzCCIPWallet::SetMintSeen(const CBigNum& bnValue, const int& nHeight, const uint256& txid, const CoinDenomination& denom)
{
    ...
    CBigNum bnRandomness;
    CKey key;
    SeedToZCCIP(seedZerocoin, bnValueGen, bnSerial, bnRandomness, key);
...
void CzCCIPWallet::SeedToZCCIP(const uint512& seedZerocoin, CBigNum& bnValue, CBigNum& bnSerial, CBigNum& bnRandomness, CKey& key)
{
...
uint512 CzCCIPWallet::GetZerocoinSeed(uint32_t n)
{
...
void CzCCIPWallet::GenerateDeterministicZCCIP(CoinDenomination denom, PrivateCoin& coin, CDeterministicMint& dMint, bool fGenerateOnly)
{
...
void CzCCIPWallet::GenerateMint(const uint32_t& nCount, const CoinDenomination denom, PrivateCoin& coin, CDeterministicMint& dMint)
{
...
bool CzCCIPWallet::RegenerateMint(const CDeterministicMint& dMint, CZerocoinMint& mint)
{
