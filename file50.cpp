...
main(int ac, const char* av[])
{
    ...
    // set  ultrapoint log output level
    ...
    if (testnet && deamon_url == "http:://127.0.0.1:17090")
    {
        deamon_url = "http:://127.0.0.1:27090";
    }
    ...
    ([&](const crow::request& req) {
        ...
        {
            return string("upx address, viewkey or tx hash not provided");
        }
    ...
    CROW_ROUTE(app, "/prove").methods("POST"_method)
        ([&](const crow::request& req) {
            ...
            {
                return string("upx address, tx private key or "
